...
barreiras = {
    equipe: [threading.Barrier(corredores_equipe) for _ in range(num_pontos)] # Criamos uma barreira para cada equipe e para cada ponto
...
def corredor(equipe_id, corredor_id):
    print(f"Equipe {equipe_id+1} - Corredor {corredor_id + 1} pronto no ponto A")
    ...
    for ponto in range(num_pontos):
        ...
        print(f"Equipe {equipe_id+1} - Corredor {corredor_id + 1} chegou ao ponto {letra_ponto}")
        ...
        if ponto < num_pontos - 1:
            print(f"Equipe {equipe_id+1} - Corredor {corredor_id + 1} partiu do ponto {letra_ponto}")
        else:
            print(f"Equipe {equipe_id+1} - Corredor {corredor_id + 1} finalizou a corrida no ponto {letra_ponto}")
